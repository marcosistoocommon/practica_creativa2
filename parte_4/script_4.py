...
if cmd == "install":
    ...
    subprocess.run("sudo rm -f /run/docker.sock /run/docker.socket 2>/dev/null || true", shell=True)
    ...
    subprocess.run("curl -L https://github.com/kubernetes-sigs/cri-tools/releases/download/v1.30.0/crictl-v1.30.0-linux-amd64.tar.gz -o /tmp/crictl.tar.gz && sudo tar -xzf /tmp/crictl.tar.gz -C /usr/local/bin && rm /tmp/crictl.tar.gz", shell=True)
