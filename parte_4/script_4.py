...
if cmd == "install":
    ...
    subprocess.run("curl -L https://github.com/Mirantis/cri-dockerd/releases/download/v0.3.14/cri-dockerd-0.3.14.amd64.tgz -o /tmp/cri-dockerd.tgz && sudo tar -xzf /tmp/cri-dockerd.tgz -C / && rm /tmp/cri-dockerd.tgz && sudo install -o root -g root -m 0755 /usr/local/bin/cri-dockerd /usr/local/bin/cri-dockerd", shell=True)
    subprocess.run("curl -L https://raw.githubusercontent.com/Mirantis/cri-dockerd/master/packaging/systemd/cri-docker.service -o /tmp/cri-docker.service && sudo mv /tmp/cri-docker.service /etc/systemd/system/ && sudo systemctl daemon-reload", shell=True)
    subprocess.run("curl -L https://raw.githubusercontent.com/Mirantis/cri-dockerd/master/packaging/systemd/cri-docker.socket -o /tmp/cri-docker.socket && sudo mv /tmp/cri-docker.socket /etc/systemd/system/ && sudo systemctl daemon-reload", shell=True)
    subprocess.run("sudo systemctl enable cri-docker && sudo systemctl start cri-docker", shell=True)
