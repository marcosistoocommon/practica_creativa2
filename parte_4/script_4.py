...
if cmd == "install":
    ...
    subprocess.run("docker build -t 17/reviews-v1 --build-arg service_version=v1 --build-arg enable_ratings=false --build-arg star_color=black .", shell=True)
    subprocess.run("docker build -t 17/reviews-v2 --build-arg service_version=v2 --build-arg enable_ratings=true --build-arg star_color=black .", shell=True)
    subprocess.run("docker build -t 17/reviews-v3 --build-arg service_version=v3 --build-arg enable_ratings=true --build-arg star_color=red .", shell=True)
    os.chdir("../../../..")
    subprocess.run("docker build -f Dockerfile.productpage -t 17/productpage .", shell=True)
    subprocess.run("docker build -f Dockerfile.ratings -t 17/ratings .", shell=True)
    subprocess.run("docker build -f Dockerfile.details -t 17/details .", shell=True)
    
    subprocess.run(f"docker tag 17/productpage gcr.io/{PROJECT_ID}/productpage:latest", shell=True)
    subprocess.run(f"docker tag 17/ratings gcr.io/{PROJECT_ID}/ratings:latest", shell=True)
    subprocess.run(f"docker tag 17/details gcr.io/{PROJECT_ID}/details:latest", shell=True)
    subprocess.run(f"docker tag 17/reviews-v1 gcr.io/{PROJECT_ID}/reviews:v1", shell=True)
    subprocess.run(f"docker tag 17/reviews-v2 gcr.io/{PROJECT_ID}/reviews:v2", shell=True)
    subprocess.run(f"docker tag 17/reviews-v3 gcr.io/{PROJECT_ID}/reviews:v3", shell=True)
    
    subprocess.run(f"docker push gcr.io/{PROJECT_ID}/productpage:latest", shell=True)
    subprocess.run(f"docker push gcr.io/{PROJECT_ID}/ratings:latest", shell=True)
    subprocess.run(f"docker push gcr.io/{PROJECT_ID}/details:latest", shell=True)
    subprocess.run(f"docker push gcr.io/{PROJECT_ID}/reviews:v1", shell=True)
    subprocess.run(f"docker push gcr.io/{PROJECT_ID}/reviews:v2", shell=True)
    subprocess.run(f"docker push gcr.io/{PROJECT_ID}/reviews:v3", shell=True)
